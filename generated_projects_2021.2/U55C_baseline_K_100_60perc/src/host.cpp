...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    size_t HBM_centroid_vectors0_len = 2 * centroids_per_partition_even * D * sizeof(float) / sizeof(uint32_t);
    size_t HBM_centroid_vectors1_len = 2 * centroids_per_partition_even * D * sizeof(float) / sizeof(uint32_t);
    size_t HBM_centroid_vectors2_len = 2 * centroids_per_partition_even * D * sizeof(float) / sizeof(uint32_t);
    size_t HBM_centroid_vectors3_len = 2 * centroids_per_partition_even * D * sizeof(float) / sizeof(uint32_t);
    size_t HBM_centroid_vectors4_len = (centroids_per_partition_even + centroids_per_partition_last_PE) * D * sizeof(float) / sizeof(uint32_t);
    ...
    size_t HBM_centroid_vectors0_size =  HBM_centroid_vectors0_len * sizeof(uint32_t);
    size_t HBM_centroid_vectors1_size =  HBM_centroid_vectors1_len * sizeof(uint32_t);
    size_t HBM_centroid_vectors2_size =  HBM_centroid_vectors2_len * sizeof(uint32_t);
    size_t HBM_centroid_vectors3_size =  HBM_centroid_vectors3_len * sizeof(uint32_t);
    size_t HBM_centroid_vectors4_size =  HBM_centroid_vectors4_len * sizeof(uint32_t);
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_centroid_vectors0(HBM_centroid_vectors0_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_centroid_vectors1(HBM_centroid_vectors1_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_centroid_vectors2(HBM_centroid_vectors2_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_centroid_vectors3(HBM_centroid_vectors3_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_centroid_vectors4(HBM_centroid_vectors4_len, 0);
...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        gt_vec_ID[2 + i] = raw_gt_vec_ID[i * 1000];
    }
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(err, cl::Buffer buffer_HBM_centroid_vectors0(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_centroid_vectors0_size, HBM_centroid_vectors0.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_centroid_vectors1(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_centroid_vectors1_size, HBM_centroid_vectors1.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_centroid_vectors2(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_centroid_vectors2_size, HBM_centroid_vectors2.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_centroid_vectors3(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_centroid_vectors3_size, HBM_centroid_vectors3.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_centroid_vectors4(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_centroid_vectors4_size, HBM_centroid_vectors4.data(), &err));
