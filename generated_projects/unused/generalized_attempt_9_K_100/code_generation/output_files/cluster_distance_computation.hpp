...
void load_vector_quantizer_from_HBM(
    ...
    // the speed of the computation PE is consuming one 512-bit sub-vector
    //   per 2 CC, so we use 1 HBM channel to store the indexes in 2 vectors

    // the HBM channel contain centroids_per_partition_A + centroids_per_partition_B
    //   vectors in total
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        for (int c = 0; c < centroids_per_partition_A; c++) {
            ...
            if (c < centroids_per_partition_B) { // write A and B

                int start_addr_HBM = c * 8 * 2; // each 512-bit = 16 numbers, 1 vec=8 addresses
                // Manually unroll 16, auto-unroll doesn't work well
                for (int d = 0; d < D / 16; d++) {
#pragma HLS pipeline II=1
                    ap_uint512_t cell_centroids_uint = HBM_centroid_vectors[start_addr_HBM + d];
                    s_HBM_centroid_vectors_A.write(cell_centroids_uint);
                }
                for (int d = 0; d < D / 16; d++) {
#pragma HLS pipeline II=1
                    ap_uint512_t cell_centroids_uint = HBM_centroid_vectors[start_addr_HBM + D / 16 + d];
                    s_HBM_centroid_vectors_B.write(cell_centroids_uint);
                }
                int cell_ID_A = start_cell_ID + 2 * c;
                int cell_ID_B = start_cell_ID + 2 * c + 1;
                s_cell_ID_A.write(cell_ID_A);
                s_cell_ID_B.write(cell_ID_B);
            }
            else { // write A only
                int start_addr_HBM = centroids_per_partition_B * 8 * 2 + (c - centroids_per_partition_B) * 8; // each 512-bit = 16 numbers, 1 vec=8 addresses
                // Manually unroll 16, auto-unroll doesn't work well
                for (int d = 0; d < D / 16; d++) {
#pragma HLS pipeline II=1
                    ap_uint512_t cell_centroids_uint = HBM_centroid_vectors[start_addr_HBM + d];
                ...
                int cell_ID_A = start_cell_ID + 2 * centroids_per_partition_B + (c - centroids_per_partition_B);
                s_cell_ID_A.write(cell_ID_A);
            }
        }
    }
