...
{
    if (argc != 7) {
        std::cout << "Usage: " << argv[0] << " <XCLBIN File> <OPQ_enable> <data directory> <ground truth dir>" << std::endl;
		return EXIT_FAILURE;
	}
    ...
    int nlist = NLIST;
    int nprobe = NPROBE;
    bool OPQ_enable = (bool) std::stoi(argv[2]);

    std::string data_dir_prefix = argv[3];
