...
if config["STAGE2_ON_CHIP"] == False:
    for i in range(config["PE_NUM_CENTER_DIST_COMP"]):
        if i != config["PE_NUM_CENTER_DIST_COMP"] - 1:
            template_fill_dict["HBM_centroid_vectors_len"] += \
                "    size_t HBM_centroid_vectors{i}_len = centroids_per_partition_even * D * sizeof(float) / sizeof(ap_uint512_t);\n".format(
                    i=i)
        else:
            template_fill_dict["HBM_centroid_vectors_len"] += \
                "    size_t HBM_centroid_vectors{i}_len = centroids_per_partition_last * D * sizeof(float) / sizeof(ap_uint512_t);\n".format(
                    i=i)
        template_fill_dict["HBM_centroid_vectors_size"] += \
        ...
        template_fill_dict["HBM_centroid_vectors_memcpy"] += """
    int HBM_centroid_vectors_start_addr_{i} = {i} * centroids_per_partition_even * D * sizeof(float);
    memcpy(&HBM_centroid_vectors{i}[0], HBM_vector_quantizer_char + HBM_centroid_vectors_start_addr_{i}, HBM_centroid_vectors{i}_size);\n""".format(i=i)
        template_fill_dict["HBM_centroid_vectorsExt"] += \
