...
<--buffer_HBM_embedding_set_krnl_arg-->
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 0, buffer_HBM_info_start_addr_and_scanned_entries_every_cell_and_last_element_valid));
    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 1, buffer_HBM_query_vectors));
    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 2, buffer_HBM_vector_quantizer));
    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 3, buffer_HBM_product_quantizer));
    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 4, buffer_HBM_OPQ_matrix));

    OCL_CHECK(err, err = krnl_vector_add.setArg(<--HBM_CHANNEL_NUM--> + 5, buffer_output));
