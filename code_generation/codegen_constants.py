...
if config["PE_NUM_CENTER_DIST_COMP"] > 1:
    ...
    CENTROIDS_PER_PARTITION_EVEN_MAX = int(np.ceil(
        template_fill_dict["NLIST_MAX"] / template_fill_dict["PE_NUM_CENTER_DIST_COMP"]))
    CENTROIDS_PER_PARTITION_LAST_PE_MAX = \
        template_fill_dict["NLIST_MAX"] - \
        PE_NUM_CENTER_DIST_COMP_EVEN * CENTROIDS_PER_PARTITION_EVEN_MAX
    template_fill_dict["stage_2_specification"] = """
#define PE_NUM_CENTER_DIST_COMP_EVEN {PE_NUM_CENTER_DIST_COMP_EVEN}
#define CENTROIDS_PER_PARTITION_EVEN_MAX {CENTROIDS_PER_PARTITION_EVEN_MAX}
#define CENTROIDS_PER_PARTITION_LAST_PE_MAX {CENTROIDS_PER_PARTITION_LAST_PE_MAX}
""".format(PE_NUM_CENTER_DIST_COMP_EVEN=PE_NUM_CENTER_DIST_COMP_EVEN,
    CENTROIDS_PER_PARTITION_EVEN_MAX=CENTROIDS_PER_PARTITION_EVEN_MAX,
    CENTROIDS_PER_PARTITION_LAST_PE_MAX=CENTROIDS_PER_PARTITION_LAST_PE_MAX)
else:
    template_fill_dict["stage_2_specification"] = ""
...
if config["PE_NUM_TABLE_CONSTRUCTION"] > 1:
    ...
    NPROBE_PER_TABLE_CONSTRUCTION_PE_LARGER = int(np.ceil(
        template_fill_dict["NPROBE"] / template_fill_dict["PE_NUM_TABLE_CONSTRUCTION"]))
    PE_NUM_TABLE_CONSTRUCTION_SMALLER = 1
    NPROBE_PER_TABLE_CONSTRUCTION_PE_SMALLER = template_fill_dict["NPROBE"] - \
        PE_NUM_TABLE_CONSTRUCTION_LARGER * NPROBE_PER_TABLE_CONSTRUCTION_PE_LARGER
    assert NPROBE_PER_TABLE_CONSTRUCTION_PE_SMALLER > 0
...
""".format(PE_NUM_TABLE_CONSTRUCTION_LARGER=PE_NUM_TABLE_CONSTRUCTION_LARGER,
    ...
    NPROBE_PER_TABLE_CONSTRUCTION_PE_LARGER=NPROBE_PER_TABLE_CONSTRUCTION_PE_LARGER,
    NPROBE_PER_TABLE_CONSTRUCTION_PE_SMALLER=NPROBE_PER_TABLE_CONSTRUCTION_PE_SMALLER)
else:
    template_fill_dict["stage_4_specification"] = ""
